...
It is easy to see Docker as just a faster VM environment or a faster Vagrant (which is a way to programmatically create VMs). I've seen companies that have mastered the deployment of VMs (i.e. they have become really good at automating the creation and deployment of multi-gigabyte VMs) completely dismiss the technology that Docker offers as just another IT fad.
...
The Docker team has published light orchestration frameworks called [Docker Compose](https://docs.Docker.com/compose/) and [Docker Swarm](https://docs.Docker.com/engine/swarm/). Whilst both solutions are relatively effective and have their share of pros and cons,[Kubernetes](https://en.wikipedia.org/wiki/Kubernetes) is by far the most widely used container orchestration mechanism in production environemnts today.
